...
namespace Cap24Team3.Controllers
{
    public class ThongBaosController : Controller
    {
        ...
        public ActionResult TaoThongBao()
        {
            ...
            if (mail != "")
            {
                ViewData["DSLop"] = db.LopQuanLies.ToList();
                ViewData["DSKhoa"] = db.KhoaDaoTaos.ToList();
                ...
                if (ModelState.IsValid)
                {
                    if (idLop != null)
                    {
                        for (int i = 0; i < idLop.Length; i++)
                        {
                            ...
                            var listsv = db.SinhViens.Where(s => s.LopQuanLy.ID == listLop.ID);
                    ...
                    if (idKhoa != null)
                    {
                        for (int i = 0; i < idKhoa.Length; i++)
                        {
                            var listKhoa = db.KhoaDaoTaos.Find(idKhoa[i]);
                            var listsv = db.SinhViens.Where(s => s.KhoaDaoTao.ID == listKhoa.ID);
                            foreach (var sv in listsv)
                            {
                                string mail = sv.Email_1;
                                var thongBao = new ThongBao();
                    ...
                    if (idNganh != null)
                    {
                        for (int i = 0; i < idNganh.Length; i++)
                        {
                            ...
                            var listsv = db.SinhViens.Where(s => s.NganhDaoTao.ID == listNganh.ID);
                            foreach (var sv in listsv)
                            {
                                string mail = sv.Email_1;
                                var thongBao = new ThongBao();
